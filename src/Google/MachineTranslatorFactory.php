        ...
        try {
            ...
            if (empty($config['apiKey'])) {
                throw new MachineTranslatorCreationException(
                    message: sprintf('Google translator "%s" requires a non-empty "apiKey" config value.', $name)
                );
            }
            ...
            throw new MachineTranslatorCreationException(
                message: sprintf('Failed creating Google translator "%s": %s', $name, $e->getMessage()),
                code: 0,
                previous: $e,
            );
        }
